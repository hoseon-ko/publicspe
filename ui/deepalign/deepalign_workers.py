...
class _BgCaptureWorker(QObject):
    ...
    def run(self):
        try:
            ...
            frames = self._acquire_fn(_progress_cb, _should_stop)

            if frames and not self._stop:
                # n>1일 때만 첫 프레임 스킵
                if len(frames) > 1 and self._n > 1:
                    valid_frames = frames[1:]
                else:
                    valid_frames = frames
                
                if valid_frames:
                    orig_dtype = np.asarray(valid_frames[0]).dtype
                    accum = np.zeros_like(valid_frames[0], dtype=np.float64)
                    for f in valid_frames:
                        accum += np.asarray(f).astype(np.float64)
                    
                    avg = accum / len(valid_frames)
                    if np.issubdtype(orig_dtype, np.integer):
                        info = np.iinfo(orig_dtype)
                        avg = np.clip(avg, info.min, info.max)
                    self.finished.emit(avg.astype(orig_dtype))
                else:
                    self.error.emit("No valid frames for background")
            elif not frames:
