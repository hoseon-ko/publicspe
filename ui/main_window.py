...
class MainWindow(QMainWindow):
    ...
    def _on_grid_frame_clicked(self, filepath: str, frame_idx: int):
        ...
        if spe_item is None:
            print(f"[DEBUG] find_item failed for: {filepath}")
            print(f"[DEBUG] known files: {[i.filepath for i in self.file_list_panel._items]}")
            return
        self._current_spe = spe_item
        frame = self._extract_frame(spe_item.spe_obj, frame_idx)
        if frame is not None:
            self.image_viewer.set_image(frame)   # set_image (뷰 유지)
            self.file_list_panel.select_file(filepath)
            self.file_list_panel.set_frame(frame_idx)
            self._update_status(spe_item, frame_idx, frame)
        else:
            print(f"[DEBUG] frame extract failed idx={frame_idx}")
    ...
    def _on_space_pressed(self):
        # 그리드 패널에 포커스 있으면 스페이스바 리셋 무시
        focused = self.focusWidget()
        if focused is not None:
            # scroll_thumb 또는 list_widget 이 포커스 가지고 있으면 무시
            if focused is self.frame_grid_panel.scroll_thumb or \
               focused is self.frame_grid_panel.list_widget:
                return
