...
methods = """
    ...
    def __init__(self, processor: ImageProcessor, parent=None):
        ...
        self._proc = processor; self._camera = None; self._caps = None; self._temp_thread = None; self._cmd_thread = None; self._cmd_worker = None
        self._settings = QSettings("SpeAnalyze", "CameraPanel"); self._sections = []
        self._build_ui(); self._load_settings(); self._set_connected(False)

    def stop_polling(self):
        if self._temp_thread and self._temp_thread.isRunning(): self._temp_thread.stop(); self._temp_thread.wait()
        if self._cmd_thread and self._cmd_thread.isRunning(): self._cmd_thread.quit(); self._cmd_thread.wait()

    def _build_ui(self):
        layout = QVBoxLayout(self); layout.setContentsMargins(6,6,6,6); layout.setSpacing(6)
        grp_dev = CollapsibleSection("CAMERA DEVICE", accent=C_ACCENT); gd = grp_dev.content_layout()
        row_t = QHBoxLayout(); lbl_t = QLabel("Type:"); lbl_t.setStyleSheet(_LBL_STYLE)
        ...
        gd.addWidget(self.btn_snap); self.bar_snap_progress = QProgressBar(); self.bar_snap_progress.setFixedHeight(4); self.bar_snap_progress.setTextVisible(False); self.bar_snap_progress.setStyleSheet("QProgressBar { background: transparent; border: none; } QProgressBar::chunk { background: #ffe66d; border-radius: 2px; }"); gd.addWidget(self.bar_snap_progress); self._sections.append(grp_dev); layout.addWidget(grp_dev)
        self.grp_config = CollapsibleSection("CAMERA CONFIGURATION", accent=C_ACCENT); gc = self.grp_config.content_layout()
        re = QHBoxLayout(); le = QLabel("Exposure (ms):"); le.setStyleSheet(_LBL_STYLE); le.setFixedWidth(100); self.spin_exposure = QDoubleSpinBox(); self.spin_exposure.setRange(0.01, 1000000); self.spin_exposure.setStyleSheet(_SPIN_STYLE); self.btn_apply_exp = QPushButton("SET"); self.btn_apply_exp.setFixedWidth(45); self.btn_apply_exp.setStyleSheet(_BTN_STYLE)
        re.addWidget(le); re.addWidget(self.spin_exposure, 1); re.addWidget(self.btn_apply_exp); gc.addLayout(re)
        self.row_frames = QWidget(); rf = QHBoxLayout(self.row_frames); rf.setContentsMargins(0,0,0,0); lf = QLabel("Frames:"); lf.setStyleSheet(_LBL_STYLE); lf.setFixedWidth(100); self.spin_readout_count = QSpinBox(); self.spin_readout_count.setRange(1, 1000000); self.spin_readout_count.setStyleSheet(_SPIN_STYLE); self.btn_apply_frames = QPushButton("SET"); self.btn_apply_frames.setFixedWidth(45); self.btn_apply_frames.setStyleSheet(_BTN_STYLE)
        rf.addWidget(lf); rf.addWidget(self.spin_readout_count, 1); rf.addWidget(self.btn_apply_frames); gc.addWidget(self.row_frames); self.div_config = QFrame(frameShape=QFrame.Shape.HLine, styleSheet="color: #0f3460;"); gc.addWidget(self.div_config); self.container_temp = QWidget(); lt = QVBoxLayout(self.container_temp); lt.setContentsMargins(0,4,0,4); rt = QHBoxLayout(); ltp = QLabel("Setpoint (°C):"); ltp.setStyleSheet(_LBL_STYLE); ltp.setFixedWidth(100); self.spin_temp = QDoubleSpinBox(); self.spin_temp.setRange(-100, 50); self.spin_temp.setStyleSheet(_SPIN_STYLE); self.btn_apply_temp = QPushButton("SET"); self.btn_apply_temp.setFixedWidth(45); self.btn_apply_temp.setStyleSheet(_BTN_STYLE)
        rt.addWidget(ltp); rt.addWidget(self.spin_temp, 1); rt.addWidget(self.btn_apply_temp); lt.addLayout(rt); self.lbl_temp_status = QLabel("Reading: —"); self.lbl_temp_status.setStyleSheet(f"color: #a0c8ff; font-family: '{Fonts.MONO}'; font-size: 12px; padding-left: 104px;"); lt.addWidget(self.lbl_temp_status); gc.addWidget(self.container_temp); self.container_adc = QWidget(); la = QVBoxLayout(self.container_adc); la.setContentsMargins(0,4,0,4); self._adc_combos = {}
        for k, l in [("adc_quality", "Quality"), ("adc_speed", "Speed"), ("adc_analog_gain", "Gain"), ("bit_depth", "Depth")]:
            r = QHBoxLayout(); lb = QLabel(f"{l}:"); lb.setStyleSheet(_LBL_STYLE); lb.setFixedWidth(100); cb = QComboBox(); cb.setStyleSheet(COMBO_STYLE); r.addWidget(lb); r.addWidget(cb, 1); la.addLayout(r); self._adc_combos[k] = cb
        self.btn_apply_adc = QPushButton("APPLY ADC SETTINGS"); self.btn_apply_adc.setStyleSheet(_BTN_STYLE); la.addWidget(self.btn_apply_adc); gc.addWidget(self.container_adc); self._sections.append(self.grp_config); layout.addWidget(self.grp_config)
        layout.addStretch(); self._connect_signals()
    ...
    def attach_camera(self, camera):
        ...
        self.container_temp.setVisible(self._caps.has_temperature); self.container_adc.setVisible(self._caps.has_adc)
        ...
        if self._caps.has_temperature:
            try:
                ...
                if mn is not None: self.spin_temp.setMinimum(mn); self.spin_temp.setMaximum(mx)
            except: pass
    ...
    def _run_sdk(self, fn, ok=None, err="Error", btn=None):
        ...
        t = QThread(); w = _CameraCommandWorker(fn); self._cmd_thread = t; w.moveToThread(t); t.started.connect(w.run)
    ...
    def update_centroid(self, x, y, bright, fps, snr, mean):
        if hasattr(self, 'lbl_cx'):
            self.lbl_cx.setText(f"cX: {x:.2f}" if x is not None else "cX: —")
            self.lbl_cy.setText(f"cY: {y:.2f}" if y is not None else "cY: —")
            self.lbl_bright.setText(f"Brightness: {bright:.1f}" if bright is not None else "Brightness: —")
            self.lbl_fps.setText(f"FPS: {fps:.1f}" if fps is not None else "FPS: —")
            self.lbl_snr.setText(f"SNR: {snr:.1f}" if snr is not None else "SNR: —")
            self.lbl_mean.setText(f"Mean: {mean:.1f}" if mean is not None else "Mean: —")

"""

# Now find where the original file's PROCESSING group starts
original_processing_content = ""
found_processing = False
for line in lines:
    if "self.grp_proc = CollapsibleSection" in line or "# ── 이진화 / Centroid 그룹" in line:
        found_processing = True
    if found_processing:
        original_processing_content += line

# Final combine
full = "".join(header) + methods + original_processing_content
